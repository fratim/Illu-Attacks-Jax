...
def _eye(sys: brax.System, state: brax.State) -> List[float]:
  ...
  return list(state.x.pos[0, :] + jp.array(off))
...
def render_arrays(sys: brax.System,
                 states: brax.State,
                 width: int,
                 height: int,
                 light: Optional[Light] = None,
  ...
  # instances = _with_state(objs, state.x.concatenate(base.Transform.zero((1,))))
  instances = _with_state(objs, states.x.concatenate(base.Transform.zero((1,))))


  if camera is None:
    camera = get_camera(sys, states, width, height, ssaa)

  img = render_instances(instances=instances,width=width * ssaa,height=height * ssaa,camera=camera,light=light,shadow=shadow,camera_target=get_target(states),enable_shadow=enable_shadow)
