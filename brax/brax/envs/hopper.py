...
class Hopper(PipelineEnv):
  ...
  * v0: Initial versions release (1.0.0)
  ...
  def __init__(
    ...
    init_obs_low, init_obs_high = self._get_initobs_low_high()
    self.init_obs_low = init_obs_low
    self.init_obs_high = init_obs_high
  ...
  def _get_initobs_low_high(self):

    low, hi = -5*self._reset_noise_scale, 5*self._reset_noise_scale
    qpos_low = self.sys.init_q + low
    qpos_high = self.sys.init_q + hi

    qvel_low = jp.ones((self.sys.qd_size(),)) * low
    qvel_high = jp.ones((self.sys.qd_size(),)) * hi

    pipeline_state_low = self.pipeline_init(qpos_low, qvel_low)
    pipeline_state_high = self.pipeline_init(qpos_high, qvel_high)

    obs_low = self._get_obs(pipeline_state_low)
    obs_high = self._get_obs(pipeline_state_high)

    return obs_low, obs_high

  
  def get_pipeline_state(self, obs: jax.Array) -> base.State:
    """Returns the pipeline state."""
    if self._exclude_current_positions_from_observation:
      qpos = jp.concatenate((jp.zeros((1,)), obs[:(self.sys.q_size()-1)]))
      qvel = obs[(self.sys.q_size()-1):]
    else:
      qpos = obs[:self.sys.q_size()]
      qvel = obs[self.sys.q_size():]

    return self.pipeline_init(qpos, qvel)
  
  def simulate_step(self, obs, action) -> State:
    """Simulates a step from a given observation and action"""
    pipeline_state0 = self.get_pipeline_state(obs)
